-- CORREÇÃO DEFINITIVA: Ao deletar transação fixa com filhas concluídas
-- DELETAR o pai e apenas "orfanar" as filhas concluídas (parent_transaction_id = NULL)
-- Assim as filhas preservadas não aparecem na página Planejamento, apenas na de Transações
...
BEGIN
  ...
  -- =====================================
  -- SCOPE: ALL - Deletar TUDO ou preservar apenas concluídas (órfãs)
  -- =====================================
  IF p_scope = 'all' THEN
    IF v_completed_children_count > 0 THEN
      -- TEM FILHAS CONCLUÍDAS:
      -- 1. Deletar TODAS as filhas PENDENTES
      -- 2. "Orfanar" as filhas CONCLUÍDAS (parent_transaction_id = NULL)
      -- 3. Deletar o PAI completamente (para não aparecer na página Transações)
      ...
      -- PASSO 2: "Orfanar" as filhas CONCLUÍDAS
      -- Isso mantém elas na página Transações com badge "Fixa"
      -- mas remove da página Planejamento (pois não têm parent)
      UPDATE transactions
      ...
      RETURN QUERY SELECT true, v_deleted_count, 
        format('Deleted %s pending + parent, preserved %s completed (orphaned)', 
