-- CORREÇÃO: Ao preservar filhas concluídas, NÃO deletar o pai
-- Apenas marcar com is_fixed = false E status = 'completed' para que:
-- 1. Não apareça na página Planejamento (is_fixed = false)
-- 2. Não apareça na página Transações (é um "pai" sem parent_transaction_id E is_fixed != true)
--    O filtro é: parent_transaction_id IS NOT NULL OR is_fixed != true OR is_fixed IS NULL
--    Com is_fixed = false, ele APARECERIA... então preciso de outra abordagem

-- NOVA ABORDAGEM: Manter as filhas concluídas com parent_transaction_id intacto
-- E marcar o pai como "deletado" usando um status especial ou flag

-- Na verdade, a melhor solução é: quando há filhas concluídas,
-- simplesmente MANTER o parent_transaction_id nas filhas
-- e o PAI marcado como is_fixed = false
-- 
-- Assim:
-- - PAI: is_fixed = false, parent_transaction_id = NULL → aparece na página Transações (is_fixed != true)
--   PROBLEMA: queremos que ele NÃO apareça em lugar nenhum
--
-- SOLUÇÃO REAL: Deletar o pai E manter o parent_transaction_id nas filhas
-- Usar ON DELETE SET NULL na FK vai setar para NULL automaticamente
-- MAS nós precisamos manter NOT NULL para as filhas aparecerem na Transações
--
-- MELHOR SOLUÇÃO: Remover a FK constraint completamente para parent_transaction_id
-- e gerenciar a integridade no código

-- Por agora, vou apenas setar is_fixed = false nas filhas concluídas
-- Isso faz elas aparecerem normalmente na página Transações (como transações comuns)
-- E não aparecem na Planejamento
