-- Fix atomic_update_transaction: Corrigir tratamento de tipo 'transfer' ao editar amount
-- Problema: Quando v_type = 'transfer', o sinal do amount não estava sendo aplicado
...
BEGIN
  ...
  BEGIN
    ...
    IF p_updates ? 'amount' THEN
      ...
      -- CRÍTICO: 'transfer' e 'expense' devem ser sempre negativos
      IF v_type = 'expense' OR v_type = 'transfer' THEN
        v_amount := -ABS(v_amount);  -- Expense e Transfer sempre negativo
    ...
    -- Se for uma transferência (tem linked_transaction_id), sincronizar a transação vinculada
    IF v_linked_transaction_id IS NOT NULL THEN
      -- FIX CRÍTICO: Se o amount foi atualizado, usar o valor INVERSO para a transação vinculada
      -- Transferências devem ter valores com sinais opostos (uma negativa, outra positiva)
      IF p_updates ? 'amount' THEN
        -- INVERSÃO: Se uma transação teve amount X, a outra deve ter -X
        -- Isso garante que uma conta perde e outra ganha o mesmo valor
        v_linked_amount := -v_amount;  -- Inverte o sinal do amount já processado
      END IF;

      -- Atualizar a transação vinculada com os mesmos campos (exceto tipo e categoria)
      UPDATE transactions SET
      ...
      -- Adicionar a conta da transação vinculada às contas afetadas
