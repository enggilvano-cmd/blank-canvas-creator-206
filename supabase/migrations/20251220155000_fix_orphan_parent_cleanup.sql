-- Fix: Limpar transação pai "órfã" quando a última filha fixa é deletada
-- Se a pai tem is_fixed = false e não tem mais filhas, ela deve ser removida
...
BEGIN
  ...
  -- Determinar o ID do parent (se for filha, usar o parent; se for pai, usar o próprio id)
  ...
  IF p_scope = 'current' THEN
    ...
    -- NOVO: Se deletamos uma filha, verificar se a transação pai ficou órfã
    -- Se a pai não tem mais filhas E tem is_fixed = false, deletar a pai também
    IF v_real_parent_id IS NOT NULL THEN
      -- Contar filhas restantes
      SELECT COUNT(*) INTO v_remaining_children_count
      FROM transactions
      WHERE parent_transaction_id = v_real_parent_id AND user_id = p_user_id;
      
      -- Verificar se pai tem is_fixed = false
  ...
  ELSIF p_scope = 'all' THEN
    -- Se há filhas concluídas, fazer SOFT DELETE na transação pai
    -- para manter a referência e o badge "Fixa" nas filhas
    IF v_has_completed_children THEN
      -- Soft delete: apenas remover is_fixed do pai para tirá-lo da página Planejamento
      UPDATE transactions
      SET is_fixed = false
      WHERE id = v_parent_id AND user_id = p_user_id;
      
      -- Deletar apenas as filhas PENDENTES (concluídas são preservadas com is_fixed = true)
      WITH transactions_to_delete AS (
        SELECT id, linked_transaction_id, to_account_id, account_id
        FROM transactions
        WHERE user_id = p_user_id 
    ...
    ELSE
      -- Sem filhas concluídas: deletar tudo normalmente
      WITH transactions_to_delete AS (
        SELECT id, linked_transaction_id, to_account_id, account_id
        FROM transactions
        WHERE user_id = p_user_id 
