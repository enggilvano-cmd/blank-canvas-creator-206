-- Fix atomic_delete_transaction para NUNCA deletar transações CONCLUÍDAS
-- Corrige o bug onde filhas concluídas estavam sendo deletadas junto com as pendentes
...
DECLARE
  ...
  v_has_completed_children BOOLEAN := false;
BEGIN
  ...
  -- REGRA: Transações CONCLUÍDAS nunca podem ser deletadas por scope 'all' ou 'current-and-remaining'
  -- Verificar se há filhas concluídas
  SELECT EXISTS(
  ...
  IF p_scope = 'current' THEN
    -- Deletar apenas a transação específica (se não for concluída em contexto fixo)
    -- Para scope 'current', deletamos a transação específica independente do status
  ...
  ELSIF p_scope = 'current-and-remaining' THEN
    -- Deletar apenas transações PENDENTES com data >= data atual
    -- NUNCA deleta transações concluídas
    ...
    WITH transactions_to_delete AS (
      ...
      WHERE user_id = p_user_id 
        ...
        AND status = 'pending'  -- CRÍTICO: Apenas pendentes
    ),
    deleted AS (
      DELETE FROM transactions 
      WHERE user_id = p_user_id 
  ...
  ELSIF p_scope = 'all' THEN
    -- Deletar TODAS as transações PENDENTES da série
    -- NUNCA deleta transações concluídas
    ...
    WITH transactions_to_delete AS (
      ...
      WHERE user_id = p_user_id 
        AND (id = v_parent_id OR parent_transaction_id = v_parent_id)
        AND status = 'pending'  -- CRÍTICO: Apenas pendentes, NUNCA concluídas
    ),
    deleted AS (
      DELETE FROM transactions 
      WHERE user_id = p_user_id 
  ...
  -- Mensagem de retorno
  IF v_has_completed_children THEN
    RETURN QUERY SELECT true, v_deleted_count, 'Deleted pending transactions, completed ones preserved'::TEXT;
  ELSE
