-- Fix: Include income provisions in totals calculation
-- Previous logic excluded ALL provisions with positive amount (t.amount > 0)
-- This incorrectly excluded income provisions (which are naturally positive)
-- Only expense provisions with positive balance should be excluded (overspent)
...
Only EXPENSE provisions with positive balance are excluded (overspent provisions).
Income provisions are naturally positive and are included in calculations.
Parameters:
