-- ✅ BUG FIX #4: Corrigir Race Condition em atomic_delete_transaction
-- 
-- PROBLEMA: Risco de excluir uma transação "Pai" enquanto uma "Filha" está sendo 
-- inserida simultaneamente, gerando registros órfãos no banco.
--
-- SOLUÇÃO: Adicionar locks adequados usando SELECT ... FOR UPDATE para garantir
-- que nenhuma operação concorrente modifique as transações relacionadas durante
-- a exclusão.
