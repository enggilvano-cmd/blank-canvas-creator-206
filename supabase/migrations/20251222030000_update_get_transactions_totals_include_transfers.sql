-- Atualiza get_transactions_totals para permitir incluir transferências
-- Adiciona parâmetro p_include_transfers (default false)
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(CASE WHEN type IN ('expense', 'transfer') THEN ABS(amount) ELSE 0 END), 0)::NUMERIC as total_expenses,
    COALESCE(
      SUM(CASE WHEN type = 'income' THEN amount ELSE 0 END) - 
      SUM(CASE WHEN type IN ('expense', 'transfer') THEN ABS(amount) ELSE 0 END), 
      0
    )::NUMERIC as balance
...
GRANT EXECUTE ON FUNCTION public.get_transactions_totals(uuid, text, text, text, text, text, boolean, boolean, date, date, text, text, boolean) TO authenticated;
