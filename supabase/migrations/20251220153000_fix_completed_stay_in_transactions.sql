-- Fix: Ao preservar transações concluídas, também remover is_fixed para que não apareçam na página Planejamento
-- As transações concluídas devem permanecer apenas na página Transações
...
BEGIN
  ...
  ELSIF p_scope = 'current-and-remaining' THEN
    -- PRIMEIRO: Atualizar transações CONCLUÍDAS para remover is_fixed
    -- Isso garante que elas não apareçam na página Planejamento após a exclusão
    UPDATE transactions
    SET is_fixed = false
    ...
    -- DEPOIS: Deletar apenas transações PENDENTES
  ...
  ELSIF p_scope = 'all' THEN
    -- PRIMEIRO: Atualizar TODAS as transações CONCLUÍDAS para remover is_fixed
    -- Isso garante que elas permaneçam apenas na página Transações
    UPDATE transactions
    SET is_fixed = false
    WHERE user_id = p_user_id 
      AND parent_transaction_id = v_parent_id
  ...
  IF v_has_completed_children THEN
    RETURN QUERY SELECT true, v_deleted_count, 'Deleted pending transactions, completed ones preserved in Transactions page'::TEXT;
  ELSE
