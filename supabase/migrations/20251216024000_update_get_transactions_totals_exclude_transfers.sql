-- Atualiza get_transactions_totals para EXCLUIR transferências em todos os cenários
-- Regras:
-- - Excluir sempre: type = 'transfer'
-- - Excluir sempre: saída de transferência (to_account_id IS NOT NULL)
-- - Excluir sempre: receita espelho de transferência (type='income' AND linked_transaction_id IS NOT NULL)
-- - Excluir Saldo Inicial
-- - Excluir apenas o PAI de transações fixas (manter filhas)
-- - Excluir provisões positivas (overspent)

-- Remover versões antigas para evitar conflito de sobrecarga
...
BEGIN
  ...
  WITH filtered_transactions AS (
    ...
    WHERE t.user_id = p_user_id
      -- EXCLUIR transferências em TODOS os cenários
      AND t.type != 'transfer'
...
-- Permissões
-- Permissões já devem estar configuradas pelas migrations anteriores.
-- Evitar erro de função não única ao aplicar REVOKE/GRANT sem assinatura específica.
